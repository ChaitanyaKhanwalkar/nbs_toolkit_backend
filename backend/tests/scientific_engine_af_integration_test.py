"""Integration smoke test for Scientific Engine Steps A through F.
...
records, does not call API routes, does not rank candidates, and does not create
...
def nbs_profile(
    *,
    ...
    option = {
        ...
        "solution": solution,
        "family": "Test family",
        "description": "Fake A-F integration catalogue row",
...
def fake_nbs_provider() -> FakeNbsCatalogService:
    """Build fake NbS profiles that exercise Step E and Step F states."""
...
def build_raw_input() -> dict[str, Any]:
    """Return request-like raw input for the A-F engine chain."""
...
def run_af_chain() -> dict[str, Any]:
    """Run Steps A, B, C, D, E, and F with fake/local objects only."""
...
def assert_engine_chain_runs_through_step_f() -> None:
    """A-F should run together and preserve expected staged output."""
...
def assert_expected_treatment_groups() -> None:
    ...
    treatment_bundle = run_af_chain()["treatment_bundle"]
...
def assert_candidate_filtering_status_mix() -> None:
    ...
    candidate_bundle = run_af_chain()["candidate_bundle"]
...
def assert_matrix_includes_only_eligible_and_data_pending() -> None:
    ...
    matrix_bundle = run_af_chain()["matrix_bundle"]
...
def assert_ineligible_candidates_excluded_and_counted() -> None:
    """Step F should count excluded ineligible candidates."""
...
def assert_weights_status_not_applied() -> None:
    """Step F must not apply AHP weights."""

    matrix_bundle = run_af_chain()["matrix_bundle"]

    assert matrix_bundle.weights_status == "not_applied"


def assert_no_future_fields() -> None:
    """A-F outputs must not include final recommendation or scoring fields."""
    ...
    assert not found, f"A-F integration leaked future fields: {sorted(found)}"
...
def main() -> None:
    """Run the A-F integration smoke checks."""

    assert_engine_chain_runs_through_step_f()
    ...
    assert_ineligible_candidates_excluded_and_counted()
    assert_weights_status_not_applied()
    assert_no_future_fields()
    assert_no_api_or_recommend_route_involved()
    print("scientific engine A-F integration checks ok: no recommendation path")
